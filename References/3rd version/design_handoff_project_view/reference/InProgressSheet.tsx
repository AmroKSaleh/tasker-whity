// InProgressSheet.tsx — mobile bottom sheet listing in-progress tasks.
...
export function InProgressSheet({ featured, others = [], onClose }: Props) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ height: '70vh', paddingBottom: 'max(22px, env(safe-area-inset-bottom))', ...style }}
      >
        ...
        <div className="flex flex-col gap-3 overflow-y-auto px-4 pb-6 pt-2">
          <div className="flex items-center justify-between">
            <h3 className="m-0 font-mono text-[11px] uppercase tracking-[0.1em] text-mute">
              In progress <span className="text-ink-2">· {count}</span>
            </h3>
          ...
          {featured && (
            <div className="flex flex-col gap-2 rounded-lg border border-ink bg-ink p-3.5 text-paper">
              <span className="font-mono text-[9.5px] uppercase tracking-[0.1em] text-paper/55">{featured.sectionName}</span>
              <p className="text-[15px] font-medium leading-snug">{featured.title}</p>
              <div className="flex items-center gap-2 font-mono text-[10px] text-paper/60">
                <span className="text-accent">● {featured.dueLabel}</span>
                {featured.startedAgo && <><span>·</span><span>{featured.startedAgo}</span></>}
              </div>
          ...
          {others.map(t => (
            <div key={t.id} className="flex flex-col gap-2 rounded-lg border border-line bg-paper p-3.5">
              <span className="font-mono text-[9.5px] uppercase tracking-[0.1em] text-mute">{t.sectionName}</span>
              <p className="text-sm font-medium leading-snug">{t.title}</p>
              <div className="font-mono text-[10px] text-mute">Due {t.dueLabel}</div>
            </div>
          ))}
        </div>
      </div>
