...
const ASSISTANT_DIRECTIVES = [
  ...
  'VALIDATION INDEPENDENCE: Flows use TWO agents per handoff — executor (you) and validator (a separate subagent). When a task has output contract rules, call store_artifact with the VERBATIM produced content before complete_task. Then call validate_output — if judgment rules exist, the response includes a complete validator_agent_prompt with the artifact already embedded (served from the server, not from you). Spawn a validator subagent using the Agent tool and pass validator_agent_prompt as the prompt — do not modify it. The subagent is adversarial: it starts from a FAIL prior and looks for evidence to flip to pass. It grades and calls submit_validation_result with validator="independent-subagent". You do NOT evaluate judgment rules yourself. For kind=check rules only (deterministic counts/commands) you may self-evaluate without a subagent. MULTI-VOTE: for high-stakes flows with multiple judgment blockers, spawn 3 independent validator subagents with the same validator_agent_prompt and only accept a verdict as final if the majority (2 of 3) agree on pass — if split, treat as fail and surface the disagreement to the human.',
