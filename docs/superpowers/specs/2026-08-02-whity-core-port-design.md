...
- **There is no authoritative schema for the core tables.** The 65 Supabase migrations contain only `ALTER`s and the *newer* tables (flows, environments, orgs, webhooks, local mode). `create table projects | sections | groups | tasks` appears nowhere in the repository — the base schema was created directly in the Supabase dashboard. **Correction (recorded during D1's design):** this does not mean whity's schema must be reverse-engineered from Supabase's live DDL. whity-core is never integrated with Supabase and never reads its database — there is no customer base to preserve continuity for, and data migration is a deliberate, later, manual step (D3) via the app's own `export_project`/`import_project` UI feature. The schema whity actually needs is recoverable entirely from code already in this repository: the React frontend's data-access hooks (`V2/app/src/hooks/*.js`, `V2/app/src/lib/*.js`), the MCP tool implementations (`V2/supabase/functions/mcp/index.ts`), and the migrations' `ALTER`s — which together describe every field, type, and relationship the running app depends on, without needing a live connection to the database that stores them.
...
2. **Schema recovery.** ~~Introspect the live Supabase database~~ — **superseded, see the correction in §2 above.** Recover authoritative DDL for the board subset from the frontend hooks, the MCP tool implementations, and the migrations' `ALTER`s, not from a live database connection whity-core will never have. Review it, then author whity migrations. Everything downstream depends on this being correct.
